                    ...
                    auto& left = value["Controller"]["left"];
                    {
                        ...
                        if (DeltaTime > 0) {
                            for (int i = 0; i < 3; ++i) {
                                ...
                                LeftControllerVelocity[i + 3] = 180.0 / M_PI * (LeftControllerEuler[i] - LastLeftControllerCon[i + 3]) / DeltaTime;
                            }
                        }
                        //输出速度
                    ...
                    auto& right = value["Controller"]["right"];
                    {
                        ...
                        if (DeltaTime > 0) {
                            for (int i = 0; i < 3; ++i) {
                                ...
                                RightControllerVelocity[i + 3] = 180.0 / M_PI * (RightControllerEuler[i] - LastRightControllerCon[i + 3]) / DeltaTime;
                            }
                        }
                        //输出速度
    ...
    try {
        ...
        while (running) { // 根据您的实际循环条件调整
            ...
            if (LeftGrip != 0.0 && LeftRunFlag) {
                std::cout << "send left command" << std::endl;
                //发送速度指令
                ...
                std::cout << "send left_stop command" << std::endl;
                base_left->Stop();
            }
            
            // RightGrip
            if (RightGrip != 0.0 && RightRunFlag) {
                std::cout << "send right command" << std::endl;
                //发送速度指令
