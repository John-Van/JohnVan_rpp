...
void dynamixelControl() {
    try {
        ...
        while (dynamixel_running) {
            ...
            std::array<double, 7> localHeadsetPose;
            {
                std::lock_guard<std::mutex> lock(headsetPoseMutex);
                // 复制HeadsetPose到局部变量，避免在锁外访问共享数据
                localHeadsetPose = HeadsetPose;
            }
            
            // 使用局部变量进行计算，避免数据竞争
            Eigen::Quaterniond q(localHeadsetPose[6], localHeadsetPose[3], localHeadsetPose[4], localHeadsetPose[5]);
...
void leftUR5Control() {
    try {
        ...
        leftConnectionOK = false; // 设置连接状态标志为false
    }
}
